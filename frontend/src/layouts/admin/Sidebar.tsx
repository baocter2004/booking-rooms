...
import { adminMenuGroups } from '@/configs/admin-menu';
    ...
    <>
      ...
      >
        ...
        <div className="flex h-16 items-center gap-2 border-b px-4">
          <ShieldCheck className={cn('h-6 w-6 text-primary', shouldCollapse && 'mx-auto')} />
          {!shouldCollapse && <span className="text-lg font-semibold">Admin Panel</span>}
        </div>
        ...
        <nav className="flex-1 space-y-4 overflow-y-auto p-2">
          {adminMenuGroups.map((group, groupIndex) => (
            <div key={groupIndex}>
              ...
              {groupIndex < adminMenuGroups.length - 1 && <Separator className="mt-4" />}
            </div>
          ))}
        </nav>
      </aside>
